...
void main()
{
	...
	//float dof = abs(texture(vertexTex, texCoord).z - dfocal)* dmax;
	float dof = abs((-dnear*dfar/(dfar+(dnear-dfar)*texture(depthTex, texCoord).r)) -dfocal) * dmax; 
	...
	{
		vec2 iidx = texCoord + ts * texIdx[i] * dof; //cojo texel y le sumo coords del actual --> 1 partido del num de texeles creo
